\chapter{Ukázky zdrojových kódů}

\lstinputlisting[caption={Ukázka definice intenzí.}]{SourceCodes/Intensions.tils}

\clearpage

\lstinputlisting[caption={Ukázka implementace registrátoru v jazyce Kotlin},language=Kotlin]{SourceCodes/Registrar.kt}

\clearpage

\lstinputlisting[caption={Ukázka implementace TIL-Script funkce v jazyce Kotlin},language=Kotlin]{SourceCodes/Function.kt}

\clearpage

\lstinputlisting[caption={Rozhraní SymbolRegistrar},language=Kotlin]{SourceCodes/SymbolRegistrar.kt}

\clearpage

\lstinputlisting[caption={Třída DefaultFunction},language=Kotlin]{SourceCodes/DefaultFunction.kt}

\clearpage

\lstinputlisting[caption={Třída FunctionInterface},language=Kotlin]{SourceCodes/FunctionInterface.kt}

\clearpage

\endinput
