...
% TODO: Check if this is alright with prof. Duzi
Dalším ústupkem praktikalitě jsou nulární funkce aplikované pomocí kompozice. Kniha \textit{TIL
jako procedurální logika} uvádí následující: \textit{Množiny $o, \iota, \tau, \omega$ jsou atomické
typy, jejichž prvky jsou ne-funkce (či nulární funkce bez argumentů)}. Definuje tedy konstanty
jako nulární funkce. Tato práce ovšem v TILScriptu definuje také nekonstantní nulární funkce.
Obrazem takovýchto funkcí může být hodnota závisející na nějakém externím stavu. Tímto stavem může
být například aktuální čas nastavený na počítači, stav náhodného generátoru čísel, apod.
Alternativou by bylo využívat pro nekonstantní nulární funkce stejnou syntax jako pro konstanty.
To by ovšem mohlo působit matoucím dojmem. Navíc již dříve mohly funkce v TILScriptu záviset
nejen na svých argumentech, ale také na externím stavu, a to například provedením volné proměnné.
